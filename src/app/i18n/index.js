...
export const fetchLanguageFromBrowser = () => {
  let browserLang = FALLBACK_LANGUAGE;

  if (window.navigator.userLanguage) {
    browserLang = window.navigator.userLanguage.substring(0, 2);
  } else if (window.navigator.language) {
    browserLang = window.navigator.language.substring(0, 2);
  }

  return resources[browserLang] ? browserLang : FALLBACK_LANGUAGE;
};
...
const injectValues = (label, values = {}, plural = undefined) => {
  ...
  Object.keys(values).forEach(
    (key) => (label = label.replace(`{{${key}}}`, values[key])),
  );
